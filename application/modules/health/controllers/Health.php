...
class Health extends MX_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('mdl_health');
    ...
    function super_speciality() {
        $data['navactive'] = 3;
        $data['flash'] = $this->session->flashdata('post');
        $data['view_file'] = "super_speciality";
        $this->load->module('templates');
        $this->templates->public_bootstrap($data);
    }
    ...
    function _get_whatsappLink(){
        ...
        $query = $this->mdl_health->_get_whatsappLink();
    ...
    function get($order_by) {
        $query = $this->mdl_health->get($order_by);
        return $query;
    }

    function get_with_limit($limit, $offset, $order_by) {
        $query = $this->mdl_health->get_with_limit($limit, $offset, $order_by);
        return $query;
    }

    function get_where($id) {
        $query = $this->mdl_health->get_where($id);
        return $query;
    }

    function get_where_custom($col, $value) {
        $query = $this->mdl_health->get_where_custom($col, $value);
        return $query;
    }

    function _insert($data) {
        $this->mdl_health->_insert($data);
    }

    function _update($id, $data) {
        $this->mdl_health->_update($id, $data);
    }

    function _delete($id) {
        $this->mdl_health->_delete($id);
    }

    function count_where($column, $value) {
        $count = $this->mdl_health->count_where($column, $value);
        return $count;
    }

    function get_max() {
        $max_id = $this->mdl_health->get_max();
        return $max_id;
    }

    function _custom_query($mysql_query) {
        $query = $this->mdl_health->_custom_query($mysql_query);
        return $query;
    }
