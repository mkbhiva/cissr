...
class Rvolumes extends MX_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('mdl_rvolumes');
    ...
    function get_volume_options(){
        ...
        $mysql_query = "select * from rvolumes where volumeStatus=1 ORDER BY id DESC";
        $query = $this->_custom_query($mysql_query);

        foreach ($query->result() as $row) {
            $options[$row->id] = $row->volumeNo.', '.$row->issueNo;
        }
    ...
    function get_latest_on_volumex1(){
        ...
        $query = $this->mdl_rvolumes->get_latest_on_volumex1();
        return $query;
    ...
    function delete_notification($update_id){
        ...
        $flash_msg = "The rVolumes was deleted successfully.";
        $value = '<div class="alert alert-success" role="alert">' . $flash_msg . '</div>';
        $this->session->set_flashdata('post', $value);
        redirect('rvolumes/manage');   
    }
    ...
    function do_upload($update_id) {
        ...
        if ($submit == 'Cancel') {
            redirect('rvolumes/create/' . $update_id);
        }
    ...
    {
        $query = $this->mdl_rvolumes->_get_notifications_list($limit, $offset);
        return $query;
    }
    ...
    function listing() {
        ...
        $data['query'] = $this->mdl_rvolumes->_get_notifications_list($limit=NULL, $offset=NULL);
    ...
    function index(){
        redirect('rvolumes/listing','refresh');
    }



    function _get_notifications_by_string($string){
        $query = $this->mdl_rvolumes->_get_notifications_by_string($string);
        return $query;
    }


    function _get_related_notifications($cat_id){
        $query = $this->mdl_rvolumes->_get_related_notifications($cat_id);
        return $query;
    }
    ...
    function create() {
        ...
        if ($submit == "Submit") {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['volUrl'] = url_title($data['volumeNo'].' '.$data['issueNo']);
    ...
    function get($order_by) {
        $query = $this->mdl_rvolumes->get($order_by);
        return $query;
    }

    function get_with_limit($limit, $offset, $order_by) {
        $query = $this->mdl_rvolumes->get_with_limit($limit, $offset, $order_by);
        return $query;
    }

    function get_where($id) {
        $query = $this->mdl_rvolumes->get_where($id);
        return $query;
    }

    function get_where_custom($col, $value) {
        $query = $this->mdl_rvolumes->get_where_custom($col, $value);
        return $query;
    }

    function _insert($data) {
        $this->mdl_rvolumes->_insert($data);
    }

    function _update($id, $data) {
        $this->mdl_rvolumes->_update($id, $data);
    }

    function _delete($id) {
        $this->mdl_rvolumes->_delete($id);
    }

    function count_where($column, $value) {
        $count = $this->mdl_rvolumes->count_where($column, $value);
        return $count;
    }

    function get_max() {
        $max_id = $this->mdl_rvolumes->get_max();
        return $max_id;
    }

    function _custom_query($mysql_query) {
        $query = $this->mdl_rvolumes->_custom_query($mysql_query);
        return $query;
    }
    ...
    function issue_check($str) {
        ...
        $mysql_query = "Select * from rvolumes where volumeNo='$volumeNo' and issueNo='$str'";
        $update_id = $this->uri->segment(3);
        if (is_numeric($update_id)) {
